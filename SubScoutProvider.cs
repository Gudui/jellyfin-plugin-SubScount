...
/// </summary>
public class SubScoutProvider : ISubtitleProvider
{
    private readonly ILogger<SubScoutProvider> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="SubScoutProvider"/> class.
    /// </summary>
    /// <param name="logger">Instance of the <see cref="ILogger{SubScoutProvider}"/> interface.</param>
    /// <param name="httpClientFactory">The <see cref="IHttpClientFactory"/> for creating Http Clients.</param>
    public SubScoutProvider(ILogger<SubScoutProvider> logger, IHttpClientFactory httpClientFactory)
    {
        _logger = logger;
    }

    /// <inheritdoc/>
    public string Name => SubScoutConstants.PLUGINNAME;

    /// <inheritdoc/>
    public IEnumerable<VideoContentType> SupportedMediaTypes => SubScoutConstants.MEDIATYPES;
    ...
    public Task<SubtitleResponse> GetSubtitles(string id, CancellationToken cancellationToken)
    {
        ...
        string[] parts = id.Split(SubScoutConstants.IDSEPARATOR);
        if (parts.Length < 3)
        {
    ...
    private IEnumerable<string> MatchFile(string mediaDir, string template, IDictionary<string, string> placeholders)
    {
        ...
        foreach (string dir in dirs)
        {
            _logger.LogDebug("Finding files in {Directory} using pattern {FilePattern} (recursive)", dir, filePattern);
            files.AddRange(Directory.EnumerateFiles(dir, "*", SearchOption.AllDirectories)
                .Where(f => fileRegex.IsMatch(Path.GetFileName(f)) && !files.Contains(f)));
        }
    ...
    public Task<IEnumerable<RemoteSubtitleInfo>> Search(SubtitleSearchRequest request, CancellationToken cancellationToken)
    {
        ...
        string[] templates = SubScoutPlugin.Instance!.Configuration.Templates;
        ...
        foreach (string template in templates)
        {
            ...
            foreach (string match in MatchFile(dir, template, placeholders))
            {
                ...
                string id = string.Join(SubScoutConstants.IDSEPARATOR, ext, lang, match);
                ...
                {
                    Id = id,
                    ProviderName = SubScoutConstants.PLUGINNAME,
                    Format = ext,
